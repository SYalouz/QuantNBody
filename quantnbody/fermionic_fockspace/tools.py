...
def build_nbody_basis(n_mo, list_N_electron):
    """
...
def build_anihilation_operator_a(nbody_basis, silent=True):
    """
    ...
    N_elec_min = np.sum(nbody_basis[0])

    a = np.zeros((2*n_mo), dtype=object) 
    ...
    for p in (range(2*n_mo)): 
        for kappa in range(dim_H):
            ...
            if ref_state[p] == 0 or np.sum(ref_state) == N_elec_min:
                pass
            else :  
                # kappa_, coeff1, coeff2 = build_final_state_ad_a(np.array(ref_state), p, q, mapping_kappa)
                kappa_, coeff = build_final_state_a(np.array(ref_state), p, nbody_basis.tolist()) # #  OLDER FUNCTION KEPT FOR DEBUGGING
                a[p][kappa_, kappa] = coeff 
...
# @njit
def build_final_state_a(ref_state, p, nbody_basis):
    """
    ...
    """
    # print(ref_state)
    state_one, p1 = new_state_after_sq_fermi_op('a', p, ref_state) 
    # print(ref_state, state_one)
    # kappa_ = mapping_kappa[make_integer_out_of_bit_vector(state_one)]
    kappa_ = nbody_basis.index( state_one.tolist() ) 
    # kappa_ = np.where( nbody_basis == state_one ) 
    return kappa_, p1
