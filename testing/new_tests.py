...
def test_FERMION_BASICS():
    ...
    assert np.allclose( nbody_basis, np.load('nbodybasis_fermion_test.npy')) , 'Error in many-body basis generation for fermionic systems'
    ...
    for mode in range(2*n_mo_active):
        for mode_ in range(2*n_mo_active): 
            assert np.allclose( a_dagger_a[mode,mode_].A , test_a_dagger_a[mode,mode_].A  )  , 'Error in a_dagger_a generation for fermionic systems'

    print( nbody_basis ,  np.load('nbodybasis_fermion_test.npy')  )


def test_FERMION_MODEL_HAMILTONIAN():
    #========================================================|
    # Parameters for the simulation  
    nelec_active     = 4   #   Number of active electrons in the Active-Space  
    ...
    assert np.allclose( H.A , np.load('ham_fermionic_model_test.npy',allow_pickle=True)) , 'Error in Hamiltonian generation for fermionic model systems' 
    assert eig_en[0] == -0.18328337002617445, 'Error in groundstate energy estimation for fermionic model systems'
    assert eig_en[1] ==  1.3628124641217596  ,'Error in first excited state energy estimation for fermionic model systems'
    
    
    
    
def test_FERMION_AB_INITIO_HAMILTONIAN():
    ...
    eig_en, eig_vec = scipy.linalg.eigh( H.A )
    assert np.allclose( H.A , np.load('ham_fermionic_ab_initio_test.npy',allow_pickle=True)) , 'Error in Hamiltonian generation for fermionic ab initio systems' 
    assert eig_en[0] + E_core + E_rep_nuc  == -7.831533642199216  , 'Error in groundstate energy estimation for fermionic ab initio systems'
    assert eig_en[1] + E_core + E_rep_nuc  == -7.704470589024512  ,'Error in first excited state energy estimation for fermionic ab initio systems'
    
    

def test_BOSON_MODEL_BASICS(): 
    n_mode  = 4
    n_boson = 10  
    ...
    for mode in range(n_mode):
        for mode_ in range(n_mode): 
            assert np.allclose( a_dagger_a[mode,mode_].A , test_a_dagger_a[mode,mode_].A  )  , 'Error in groundstate energy estimation'
             
    

def test_BOSON_MODEL_HAMILTONIAN(): 
    n_mode  = 4
    n_boson = 10
    U = 1
    ...
    H = qnb.bosonic.tools.build_hamiltonian_bose_hubbard( h_,
                                                          ...
                                                          a_dagger_a ) 
    eig_en, eig_vec = scipy.linalg.eigh( H.A  )   
    assert np.allclose( H.A , np.load('ham_boson_test.npy',allow_pickle=True)) , 'Error in Hamiltonian generation for bosonic systems' 
    assert eig_en[0] == -49.276698175251404 , 'Error in groundstate energy estimation'
    assert eig_en[1] == -49.27144736991592  , 'Error in first excited state energy estimation' 

a =   test_FERMION_BASICS()
