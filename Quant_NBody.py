...
def Visualize_WFT( WFT, NBody_Basis, cutoff=0.005 ): 
    ...
    for indx in list_sorted_indx[0:8]:
        ...
        print('\t', sign_, '{:1.5f}'.format(abs(Coeffs[indx]) ), '\t|{}⟩'.format(' '.join([ str(elem) for elem in States[indx] ]).replace(" ", "") )) 
...
def Generate_H_ring_geometry(N_atoms, radius):
    ...
    theta_ini = 0.0
    H_ring_geometry = '\nH {:.16f} {:.16f} 0.'.format( radius*m.cos(theta_ini), radius*m.sin(theta_ini) ) #'H {:.16f} 0. 0.'.format( radius )  # Define the position of the first atom
    for n in range(1,N_atoms):
