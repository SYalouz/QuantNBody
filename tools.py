...
def generate_h_chain_geometry(N_atoms, dist_HH,sym=True):
    """
    ...
    for n in range(1, N_atoms):
        if sym:
            ...
            h_chain_geometry += '\nH {} 0. 0.'.format(n * dist_HH*(1+0.0001*(np.random.random()-0.5)))

    return h_chain_geometry


def generate_h_ring_geometry(N_atoms, radius,sym=True):
    """
    ...
    for n in range(1, N_atoms):
        ...
        if sym:
            ...
            h_ring_geometry += '\nH {:.16f} {:.16f} 0.'.format(radius* (1+0.0001*(np.random.random()-0.5)) * np.cos(angle_h), radius* (1+0.0001*(np.random.random()-0.5)) * np.sin(angle_h))
